 ...
 */
export default function PublicAlbumBottomBar({ albumId, embedded = false }) {
  if (!albumId) return null;
  const openInAppUrl = `pxi://album/${albumId}`;
  const year = new Date().getFullYear();

  const wrapperClass = embedded
    ? 'flex flex-col items-center px-4 pb-[calc(0.75rem+env(safe-area-inset-bottom,0px))] pt-2'
    : 'pointer-events-none fixed inset-x-0 bottom-0 z-50 flex flex-col items-center px-4 pb-[calc(0.75rem+env(safe-area-inset-bottom,0px))] pt-0 md:hidden';

  return (
    <div className={wrapperClass}>
      <div className={`flex w-full max-w-md flex-col items-center gap-2 ${embedded ? '' : 'pointer-events-auto'}`}>
        <div className="flex w-full max-w-[24rem] items-center gap-2 rounded-2xl border border-white/15 bg-black/85 px-3 py-2.5 shadow-lg backdrop-blur-md">
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[10px] text-zinc-400">
              React, comment, and post in the app — this page is read-only
            </p>
          </div>
          <a
            href={openInAppUrl}
