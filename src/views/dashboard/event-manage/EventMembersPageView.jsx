...
export default function EventMembersPageView() {
  const { user } = useAuth();
  const { event, eventId, albumId, participants, reloadParticipants, isPast } = useEventManage();
  ...
  const canManageMembers = isOwner && !isPast;
  ...
  return (
    <div className="space-y-6">
      ...
      {!isPast && activeTab === 'send' ? <EventInvitePageView initialTab="send" showTabs={false} /> : null}
      {!isPast && activeTab === 'status' ? <EventInvitePageView initialTab="status" showTabs={false} /> : null}

      {(isPast || activeTab === 'attending') ? (
        <section className="dashboard-surface overflow-hidden rounded-2xl">
          <div className="flex items-center justify-between p-5 border-b border-white/[0.06]">
            <div>
              <p className="text-[10px] font-bold tracking-[0.02em] text-zinc-500">Access list</p>
                    ...
                    {canManageMembers && !isPast && (
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <button
                          ...
                          className="pill-ghost inline-flex items-center gap-1.5 px-3 py-2 text-xs font-bold tracking-[0.02em] text-red-200 disabled:cursor-not-allowed disabled:opacity-50"
                        >
