...
export default function DataSourceBadge({ source = 'Derived', className = '', compact = false }) {
    ...
    return (
        <span
            className={`inline-flex items-center rounded-full border ${compact ? 'h-6 w-6 justify-center p-0 text-[9px]' : 'px-2.5 py-1 text-[10px]'} font-bold uppercase tracking-widest ${style} ${className}`.trim()}
