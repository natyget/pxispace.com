...
export default function FunnelChart({ data = [] }) {
    ...
    const segments = useMemo(() => {
        ...
        return data.map((stage, index) => {
            ...
            const topWidth = Math.max(220, 640 * ((Number(stage.value) || 0) / maxValue));
            const bottomWidth = nextStage ? Math.max(190, 640 * ((Number(nextStage.value) || 0) / maxValue)) : Math.max(160, topWidth * 0.7);
            const y = 18 + index * (segmentHeight + gap);
            return {
                ...stage,
                index,
