import React, { useState, useEffect } from "react";
import { Menu, X, Bell } from "lucide-react";
import { NavLink } from "react-router-dom";
import Button from "../ui/Button";

const Navbar = () => {
  const [isScrolled, setIsScrolled] = useState(false);
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);

  useEffect(() => {
    const handleScroll = () => {
      setIsScrolled(window.scrollY > 20);
    };

    window.addEventListener("scroll", handleScroll);
    return () => window.removeEventListener("scroll", handleScroll);
  }, []);

  const navLinks = [
    { name: "Home", path: "/" },
    { name: "Events", path: "/events" },
    { name: "About", path: "/about" },
  ];

  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        isScrolled || mobileMenuOpen
          ? "py-4 bg-black/60 backdrop-blur-2xl border-b border-white/5"
          : "py-8 bg-transparent"
      }`}
    >
      <div className="container mx-auto px-6 flex items-center justify-between">
        {/* Logo */}
        <NavLink
          to="/"
          className="flex items-center gap-4 cursor-pointer"
          onClick={() => setMobileMenuOpen(false)}
        >
          <div className="text-2xl font-black tracking-[0.2em] text-white">
            PXI
          </div>
        </NavLink>

        {/* Desktop Navigation */}
        <div className="hidden md:flex items-center bg-zinc-900/50 p-1.5 rounded-full border border-white/5 backdrop-blur-md">
          {navLinks.map((link) => (
            <NavLink
              key={link.path}
              to={link.path}
              onClick={() => setMobileMenuOpen(false)}
              className={({ isActive }) =>
                `px-6 py-2 rounded-full text-xs font-black uppercase tracking-widest transition-all duration-300 ${
                  isActive
                    ? "bg-pxi-purple text-white shadow-[0_0_20px_rgba(216,74,255,0.4)]"
                    : "text-zinc-500 hover:text-white"
                }`
              }
            >
              {link.name}
            </NavLink>
          ))}
        </div>

        {/* Action Buttons */}
        <div className="hidden md:flex items-center gap-4">
          <button className="p-2 text-zinc-400 hover:text-white transition-colors">
            <Bell size={20} />
          </button>

          <Button
            variant="neon"
            className="px-8 !py-2 !text-xs !tracking-[0.1em] uppercase"
          >
            Get App
          </Button>
        </div>

        {/* Mobile Toggle */}
        <button
          className="md:hidden text-white p-2"
          onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
        >
          {mobileMenuOpen ? <X size={28} /> : <Menu size={28} />}
        </button>
      </div>

      {/* Mobile Menu */}
      {mobileMenuOpen && (
        <div className="md:hidden absolute top-full left-0 w-full glass-card p-8 flex flex-col gap-6 animate-fade-up h-screen">
          {navLinks.map((link) => (
            <NavLink
              key={link.path}
              to={link.path}
              onClick={() => setMobileMenuOpen(false)}
              className={({ isActive }) =>
                `text-left text-2xl font-black uppercase tracking-widest pb-4 border-b border-white/5 ${
                  isActive ? "text-white" : "text-zinc-400"
                }`
              }
            >
              {link.name}
            </NavLink>
          ))}

          <Button
            variant="neon"
            className="w-full uppercase tracking-widest mt-4"
          >
            Download Now
          </Button>
        </div>
      )}
    </header>
  );
};

export default Navbar;
