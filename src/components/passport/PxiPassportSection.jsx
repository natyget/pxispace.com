...
export function PxiPassportSection({
    ...
    eventsMode = 'self',
    /** Dashboard / public profile show badge + title + friends; event host embed hides this */
    showHeader = true,
    className = '',
    ...
    const tierInfo = getOdysseyTierFromXp(user?.odysseyXp);
    const { badgeLetter } = getPassportLevelDisplay(user);
    const badgeTheme = getPassportLevelBadgeTheme(tierInfo.id);
                        ...
                        <div className="relative flex min-h-[36px] items-center justify-between">
                            <div className="relative z-10 flex shrink-0 items-center">
                                <HeaderPolygonBadge
                                    ...
                                    hexFill={badgeTheme.fill}
                                    hexStroke={badgeTheme.stroke}
                                    ringMuted={badgeTheme.progressTrack}
                                    ringBright={badgeTheme.progressFill}
                                />
                            </div>
