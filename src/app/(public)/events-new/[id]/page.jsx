                  ...
                  <div className="flex items-start justify-between gap-3">
                    ...
                    >
                      <span className="relative mt-0.5 flex size-5 shrink-0 overflow-hidden rounded-full bg-zinc-800">
                        {organizerAvatar ? (
                          <img className="size-full object-cover" alt="" src={organizerAvatar} />
                        ) : (
                            ...
                            <div className="group relative aspect-square w-full overflow-hidden rounded-sm bg-zinc-800">
                              {item.href ? (
                                <a target="_blank" className="relative block h-full w-full" href={item.href} rel="noopener noreferrer">
                                  {item.image ? (
                                    <img alt={item.alt} loading="lazy" width={400} height={400} decoding="async" className="h-full w-full object-cover transition-all duration-200 ease-in-out" src={item.image} />
                                  ) : (
                                ...
                                <img alt={item.alt} loading="lazy" width={400} height={400} decoding="async" className="h-full w-full object-cover" src={item.image} />
                              ) : (
                                <span className="flex h-full w-full items-center justify-center text-2xl font-medium text-zinc-500">{(item.name || '?').charAt(0).toUpperCase()}</span>
                              )}
                            </div>
                ...
                {hasHost ? (
                  <div className="rounded-xl bg-white/5 p-4 backdrop-blur-xl">
                    ...
                    <div className="mb-6 mt-10 flex flex-col items-center">
                      {organizerAvatar ? (
                        <img alt={organizerName} loading="lazy" width={200} height={200} decoding="async" className="h-52 w-52 rounded-full object-cover opacity-100 transition-opacity duration-300" src={organizerAvatar} />
                      ) : (
