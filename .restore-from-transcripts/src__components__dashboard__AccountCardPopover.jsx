...
const defaultItems = [
  { id: 'passport', label: 'PXI Passport', icon: Passport01Icon },
...
const POPOVER_BASE_STYLE = {
  ...
  zIndex: 9999,
  border: 'none',
  boxShadow: '0 8px 40px rgba(0,0,0,0.6), 0 4px 20px rgba(255,255,255,0.04)',
};
  ...
  return (
    <div className="relative" ref={rootRef}>
      <button
        ...
        className={`group inline-flex items-center glow-surface-soft text-left transition hover:bg-white/[0.06] ${
      ...
      {mounted && isOpen && popoverStyle
        ? createPortal(
            <div
              ref={popoverRef}
              style={popoverStyle}
              className="w-56 rounded-xl bg-zinc-950/98 p-1.5 backdrop-blur"
            >
              {items.map((item) => {
                const toneClass =
                  item.tone === 'danger'
